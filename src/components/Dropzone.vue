<template>
    <form class="dropzone">
        ...
        <div id="dropzone-message" style="display: none">
            <span class="dropzone-title">Drop files here or click the area to select</span>
            <span class="dropzone-info">You can upload multiple file at once</span>
        </div>
    </form>
</template>
    ...
    mounted() {
        ...
        let options = {
            ...
            accept (file, done) {
                ...
                .catch((err) => {
                    done('Faled to get an S3 signed upload URL', err)
                })
            }
        }
        ...
        //Set a signed upload URL for each file
